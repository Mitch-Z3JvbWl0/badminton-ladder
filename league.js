...
function renderAll(){
  ...
  [...new Set(fixtures.map(f=>f.Week))].forEach(week=>{
  ...
  // Standings
  let sBody=document.querySelector("#standingsTable tbody"); sBody.innerHTML="";
  ...
  // Player Stats
  let statsDiv=document.getElementById("playerStats"); statsDiv.innerHTML="";
  standings.forEach(p=>{
    let avgMargin=(p.played?(p.for-p.against)/p.played:0).toFixed(1);
    let card=document.createElement("div"); card.className="stats-card";
  ...
  new Chart(ctx1,{type:'bar',data:{labels:standings.map(p=>p.name),datasets:[{label:'Points',data:standings.map(p=>p.points),backgroundColor:'#004080'}]}});
  ...
  // Schedule (auto)
  let schedBody=document.querySelector("#scheduleTable tbody"); schedBody.innerHTML="";
  const names=standings.map(p=>p.name); if(names.length%2===1) names.push("BYE");
  const n=names.length, arr=names.slice(), rounds=n-1;
  ...
  // Profiles
  let pDiv=document.getElementById("profilesContainer"); pDiv.innerHTML="";
  Object.entries(playerProfiles).forEach(([name,prof])=>{
    let card=document.createElement("div"); card.className="profile-card";
    card.innerHTML=`<img src="${prof.image}" alt="${name}"><h3 id="profile-${name}">${name}</h3>
      <table>
